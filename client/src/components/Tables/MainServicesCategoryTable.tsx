...
interface MainServicesCategory {
    id: number;
    name: string;
}

const MainServicesCategoryTable = () => {
    const router = useRouter();
    const [categories, setCategories] = useState<MainServicesCategory[]>([]);
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_URL}/admin/mainServicesCategory/${id}`,
                {
                    method: 'DELETE',
                    headers: {
                        Authorization: `Bearer ${storedUserInfo}`,
...
export default MainServicesCategoryTable;
